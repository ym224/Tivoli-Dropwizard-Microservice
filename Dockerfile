...
ADD  build/distributions/Tivoli-Dropwizard-Microservice.tar  /
...
# add it explicitly
ADD appconfig.yml /Tivoli-Dropwizard-Microservice/

# Convenience if we ever want to log into the image and snoop around
WORKDIR /Tivoli-Dropwizard-Microservice
...
ENTRYPOINT ["/Tivoli-Dropwizard-Microservice/bin/Tivoli-Dropwizard-Microservice", "server", "appconfig.yml"]
